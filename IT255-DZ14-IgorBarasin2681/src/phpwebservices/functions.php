<?php
	...
	function updateRoom($roomName, $hasTV, $beds, $room_type_id, $id){
		global $conn;
		$rarray = array();
		if(checkIfLoggedIn()){
		$stmt = $conn->prepare("UPDATE rooms SET roomname=?, tv=?, beds=?, room_type_id=? WHERE id=?");
		$stmt->bind_param("sssii", $roomName, $hasTV, $beds, $room_type_id, $id);
		if($stmt->execute()){
