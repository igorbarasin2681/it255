...
export class LoginComponent {
	...
	onRegister(): void {
		...
		this.http.post('http://localhost/it255/IT255-DZ12-IgorBarasin2681/src/phpwebservices/registerservice.php',data,
		{headers:headers})
		.map(res => res)
		.subscribe( data => {
	...
	onLogin(): void {
		...
		let headers = new Headers();
		headers.append('Content-Type', 'application/x-www-form-urlencoded');
		this.http.post('http://localhost/it255/IT255-DZ12-IgorBarasin2681/src/phpwebservices/loginservice.php',data,
		{headers:headers})
		.map(res => res)
		.subscribe( data => {
			let obj = JSON.parse(data["_body"]);
